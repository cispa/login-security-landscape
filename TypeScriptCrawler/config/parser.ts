...
const parser = new ArgumentParser({ description: 'Crawler configuration options for visting' })
...
parser.add_argument('--test', { help: 'Run crawler in test mode', action: "store_true" })

parser.add_argument('--url', { help: 'Visit url in test mode' })
...
parser.add_argument('--foxhound', { help: 'Use Foxhound to crawl' })

parser.add_argument('--polling', { help: 'Use polling mechanism for work fetching instead of killing the crawler when no work is returned anymore' })

parser.add_argument('--forever', { help: 'Keep crawler processes alive and polling', action: "store_true" })

parser.add_argument('--fill', { help: 'Fill database with default fill data for testing', action: "store_true" })
